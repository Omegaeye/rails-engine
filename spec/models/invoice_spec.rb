...
RSpec.describe Invoice, type: :model do
  before :each do
    ...
    @invoice_item2 = InvoiceItem.create(item: @item2, invoice: @invoice8, quantity: 100, unit_price: 100)
  ...
  describe 'Class Methods' do
    it "testing invoices with successful transactions" do
      expect(Invoice.invoices_with_successful_transactions.size).to eq(32)
      expect(Invoice.invoices_with_successful_transactions.class).to eq(Array)
    end

    it "testing delete invoice" do
      expect(@item1.invoices.delete_invoice).to eq([@invoice1])
      expect(Invoice.first.id).to eq(@invoice2.id)
    end
  end
end
