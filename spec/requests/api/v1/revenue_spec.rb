...
RSpec.describe "Api::V1::Revenue::Revenues", type: :request do
  ...
  let(:valid_headers) {
    ...
    describe "GET /revenue" do
      it "Get Revenue between two dates" do
        ...
        @transaction32 = Transaction.create(invoice: @invoice32, credit_card_number: '123456789', credit_card_expiration_date: '02/02', result: "success")
          ...
          get '/api/v1/revenue?start=1970-01-01&end=2100-01-01', headers: valid_headers, as: :json
          ...
          expect(body[:data][:attributes][:revenue]).to eq("500000.0")


          get '/api/v1/revenue/merchants/1', headers: valid_headers, as: :json
          body = JSON.parse(response.body, symbolize_names: true)
      ...
      it "error out with no params" do
        get '/api/v1/revenue', headers: valid_headers, as: :json
        expect(response).to have_http_status(400)
      end

      it "error out with no start_date" do
        get '/api/v1/revenue?end=2012-03-24', headers: valid_headers, as: :json
        expect(response).to have_http_status(400)
      end

      it "error out with empty params" do
        get '/api/v1/revenue?start=&end=', headers: valid_headers, as: :json
        expect(response).to have_http_status(400)
      end

      it "error out with empty params" do
        get '/api/v1/revenue?start=2012-03-01&end=', headers: valid_headers, as: :json
        expect(response).to have_http_status(400)
      end
